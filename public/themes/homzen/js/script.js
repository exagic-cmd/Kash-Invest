"use strict";
(function() {
	//#region platform/themes/homzen/assets/js/script.js
	$(() => {
		...
		const processTemplateContent = () => {
			const processTemplate = (templateContent, data) => {
				...
				$tempDiv.find(".map-listing-item [data-href]").each(function() {
					...
					if (dataHref === "__url__" && data) $this.attr("href", data.url || data.link || "#");
					else if (dataHref && dataHref !== "__url__") $this.attr("href", dataHref);
					$this.removeAttr("data-href");
				});
				$tempDiv.find(".map-listing-item [data-src]").each(function() {
					const $this = $(this);
					const dataSrc = $this.attr("data-src");
					if (dataSrc === "__image__" && data) $this.attr("src", data.image || data.thumb || data.thumbnail || "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7");
					else if (dataSrc && dataSrc !== "__image__") $this.attr("src", dataSrc);
					$this.removeAttr("data-src");
				});
				$tempDiv.find(".map-listing-item [data-alt]").each(function() {
					const $this = $(this);
					const dataAlt = $this.attr("data-alt");
					if (dataAlt === "__name__" && data) $this.attr("alt", data.name || data.title || "");
					else if (dataAlt && dataAlt !== "__name__") $this.attr("alt", dataAlt);
					$this.removeAttr("data-alt");
				});
				$tempDiv.find(".map-listing-item [data-title]").each(function() {
					const $this = $(this);
					const dataTitle = $this.attr("data-title");
					if (dataTitle === "__name__" && data) $this.attr("title", data.name || data.title || "");
					else if (dataTitle && dataTitle !== "__name__") $this.attr("title", dataTitle);
			...
			if (typeof MutationObserver !== "undefined") new MutationObserver((mutations) => {
				...
				mutations.forEach((mutation) => {
					$(mutation.addedNodes).each(function() {
						if ($(this).hasClass("map-listing-item") || $(this).find(".map-listing-item").length > 0) hasNewMapListing = true;
					});
				});
				if (hasNewMapListing) convertMapListingAttributes();
		...
		const parallax = function() {
			if ($().parallax && isMobile.any() == null) $(".parallax").parallax("50%", .2);
		};
		const flatContentBox = function() {
			$(window).on("load resize", function() {
				...
				$(".themesflat-content-box").each(function() {
					if ($(this).data("margin")) {
						if (mode === "desktop") $(this).attr("style", "margin:" + $(this).data("margin"));
						else if (mode === "mobile") $(this).attr("style", "margin:" + $(this).data("mobilemargin"));
					}
				});
			});
		};
			...
			if ($counter.length > 0 && $(document.body).hasClass("counter-scroll")) {
				...
				$(window).scroll(function() {
					...
					if (a === 0 && $(window).scrollTop() > oTop) {
						if ($().countTo) $(".tf-counter").find(".number").each(function() {
							const to = $(this).data("to"), speed = $(this).data("speed"), dec = $(this).data("dec");
							$(this).countTo({
								to,
								speed,
								decimals: dec
		...
		const popUpLightBox = function() {
			if (typeof $.fancybox === "undefined" || !$.fancybox.defaults) return;
			$.fancybox.defaults.hash = false;
			const originalOnInit = $.fancybox.defaults.onInit;
			$.fancybox.defaults.onInit = function(instance) {
				if (typeof originalOnInit === "function") originalOnInit.call(this, instance);
				if (!instance || !instance.group || instance.group.length <= 1) return;
		...
		const showPass = function() {
			$(".show-pass").on("click", function() {
				...
				if ($(".password-field").attr("type") == "password") $(".password-field").attr("type", "text");
				else if ($(".password-field").attr("type") == "text") $(".password-field").attr("type", "password");
			});
			$(".show-pass2").on("click", function() {
				$(this).toggleClass("active");
				if ($(".password-field2").attr("type") == "password") $(".password-field2").attr("type", "text");
				else if ($(".password-field2").attr("type") == "text") $(".password-field2").attr("type", "password");
			});
			$(".show-pass3").on("click", function() {
				$(this).toggleClass("active");
				if ($(".password-field3").attr("type") == "password") $(".password-field3").attr("type", "text");
				else if ($(".password-field3").attr("type") == "text") $(".password-field3").attr("type", "password");
			});
		};
		...
		const clickSearchForm = function() {
			...
			if (widgetSearchForm.length) {
				...
				$(document).on("click", ".pull-right, .offcanvas-backdrop", function(a) {
					a.preventDefault();
					if ($(a.target).closest(".pull-right, .wd-search-form").length === 0) widgetSearchForm.removeClass("show");
				});
			}
		};
		const datePicker = function() {
			if ($("#datepicker1").length > 0) $("#datepicker1").datepicker({
				firstDay: 1,
				dateFormat: "dd/mm/yy"
			});
			if ($("#datepicker2").length > 0) $("#datepicker2").datepicker({
				firstDay: 1,
				dateFormat: "dd/mm/yy"
			});
			if ($("#datepicker3").length > 0) $("#datepicker3").datepicker({
				firstDay: 1,
				dateFormat: "dd/mm/yy"
			});
		...
		const headerFixed = function() {
			if ($("header").hasClass("header-fixed")) {
				...
				if (nav.length) {
					nav.offset().top;
					const headerHeight = nav.height(), injectSpace = $("<div>", { height: headerHeight });
					injectSpace.hide();
		...
		if ($.isFunction($.fn.niceSelect)) {
			...
			$(".form-sl .select_js, .widget-filter-search .select_js").not("#location").each(function() {
				...
				if ($select.val()) $niceSelect.addClass("has-value");
		...
		$(document).on("click", ".multi-select-trigger", function(e) {
			e.stopPropagation();
		...
		$(document).on("submit", "form.subscribe-form", (e) => {
			...
			$.ajax({
				...
				complete: () => {
					if (typeof refreshRecaptcha !== "undefined") refreshRecaptcha();
		...
		const animateHeading = () => {
			...
			function animateHeadline($headlines) {
				...
				$headlines.each(function() {
					...
					if (headline.hasClass("loading-bar")) {
						...
						var words = headline.find(".cd-words-wrapper .item-text"), width = 0;
						words.each(function() {
			...
			function hideWord($word) {
				...
				} else if ($word.parents(".animationtext").hasClass("clip")) $word.parents(".cd-words-wrapper").animate({ width: "2px" }, revealDuration, function() {
			...
			function showWord($word, $duration) {
				...
				} else if ($word.parents(".animationtext").hasClass("clip")) $word.parents(".cd-words-wrapper").animate({ width: $word.width() + 10 }, revealDuration, function() {
					setTimeout(function() {
						hideWord($word);
			...
			function showLetter($letter, $word, $bool, $duration) {
				...
				if (!$letter.is(":last-child")) setTimeout(function() {
					...
					if ($word.parents(".animationtext").hasClass("type")) setTimeout(function() {
		...
		const rangeSlider = () => {
			if (typeof wNumb === "undefined" || typeof noUiSlider === "undefined") return;
			function buildPriceRange(min, max) {
				...
				const rawLow = min + span * .08;
				const rawMid = min + span * .3;
				const stepLow = niceStep(Math.max(1e3, rawLow / 50));
				const stepMid = niceStep(Math.max(5e3, (rawMid - rawLow) / 40));
				const stepHigh = niceStep(Math.max(1e4, (max - rawMid) / 40));
			...
			const priceSlider = () => {
				...
				$("[data-bb-toggle=\"range\"]").each((index, el) => {
					...
					if ($(rangeSlider).data("currency-prefix-symbol")) moneyFormatOptions.prefix = currencySymbol + (currencyWithSpace ? " " : "");
					else moneyFormatOptions.postfix = (currencyWithSpace ? " " : "") + currencySymbol;
					...
					if (isLinear) sliderOptions.step = 1;
		...
		const Spanizer = (function() {
			const settings = { letters: $(".js-letters") };
			return {
				init: function() {
					this.bind();
				},
				...
				doSpanize: function() {
					settings.letters.html(function(i, el) {
						return `<span>${$.trim(el).split("").join("</span><span>")}</span>`;
					});
				}
			};
		})();
		if (matchMedia("only screen and (min-width: 991px)").matches) Spanizer.init();
		if ($(".thumbs-swiper-column").length > 0) {
			...
			new Swiper(".thumbs-swiper-column", {
				rtl: Theme.isRtl(),
				spaceBetween: 0,
				autoplay: {
					delay: 3e3,
		...
		if ($(".slider-sw-home2").length > 0) new Swiper(".slider-sw-home2", {
			rtl: Theme.isRtl(),
			spaceBetween: 0,
			autoplay: {
				delay: 2e3,
		...
		const initPropertiesTab = () => {
			$(document).off("click", "[data-bb-toggle=\"properties-tab\"] [data-bs-toggle=\"tab\"]").on("click", "[data-bb-toggle=\"properties-tab\"] [data-bs-toggle=\"tab\"]", (e) => {
		...
		const initProjectsTab = () => {
			$(document).off("click", "[data-bb-toggle=\"projects-tab\"] [data-bs-toggle=\"tab\"]").on("click", "[data-bb-toggle=\"projects-tab\"] [data-bs-toggle=\"tab\"]", (e) => {
				const currentTarget = $(e.currentTarget);
		...
		function cleanFormData(formDataInput) {
			...
			let queryString = formData.filter((item) => item.name !== "_token").map((item) => `${encodeURIComponent(item.name)}=${encodeURIComponent(item.value)}`);
		...
		$("[data-bb-toggle=\"detail-map\"]").each((index, element) => {
			const initDetailMap = () => {
				...
				L.tileLayer($element.data("tile-layer"), { maxZoom: $element.data("max-zoom") || 22 }).addTo(map);
				L.marker($element.data("center"), { icon: L.divIcon({
		...
		const initMap = (formData) => {
			...
			const centerFirst = $(".homeya-box[data-lat][data-lng]").filter((index, item) => $(item).data("lat") && $(item).data("lng"));
			if (centerFirst && centerFirst.length) center = [centerFirst.data("lat"), centerFirst.data("lng")];
			const map = L.map($element.prop("id"), {
				attributionControl: false,
				scrollWheelZoom: false,
				dragging: !L.Browser.mobile,
			...
			const populate = () => {
				if (typeof formData === "undefined") {
					...
					formData = {};
					if (urlParams.size > 0) for (const [key, value] of urlParams) formData[key] = value;
					else formData = { page: 1 };
				} else if (Array.isArray(formData)) formData = formData.reduce((acc, { name, value }) => {
					acc[name] = value;
					return acc;
				}, {});
				formData.page = currentPage;
				if (totalPage === 0 || currentPage <= totalPage) $.ajax({
					url: $element.data("url"),
					type: "GET",
					data: formData,
					success: ({ data, meta }) => {
						if (data.length < 1) {
							if (markers.getLayers().length > 0) try {
						...
						data.forEach((item) => {
							...
							let content = isProperty ? $("#property-map-content").html() : $("#project-map-content").html();
							content = content.replace(/* @__PURE__ */ new RegExp("__name__", "gi"), item.name).replace(/* @__PURE__ */ new RegExp("__location__", "gi"), item.location).replace(/* @__PURE__ */ new RegExp("__image__", "gi"), item.image_thumb).replace(/* @__PURE__ */ new RegExp("__price__", "gi"), item.formatted_price).replace(/* @__PURE__ */ new RegExp("__url__", "gi"), item.url).replace(/* @__PURE__ */ new RegExp("__status__", "gi"), item.status_html);
							if (isProperty) {
								if (!item.number_bedroom || item.number_bedroom === 0) content = content.replace(/<li class="map-number-of-bedroom">[\s\S]*?<\/li>/gi, "");
								else content = content.replace(/* @__PURE__ */ new RegExp("__bedroom__", "gi"), item.number_bedroom);
								if (!item.number_bathroom || item.number_bathroom === 0) content = content.replace(/<li class="map-number-of-bathroom">[\s\S]*?<\/li>/gi, "");
								else content = content.replace(/* @__PURE__ */ new RegExp("__bathroom__", "gi"), item.number_bathroom);
								if (!item.square || item.square === "" || item.square === 0) content = content.replace(/<li class="map-square">[\s\S]*?<\/li>/gi, "");
								else content = content.replace(/* @__PURE__ */ new RegExp("__square__", "gi"), item.square_text);
							}
							const marker = L.marker(L.latLng(item.latitude, item.longitude), { icon: L.divIcon({
						...
						if (meta && meta.last_page) {
							if (totalPage === 0) totalPage = meta.last_page;
							currentPage++;
							populate();
						} else if (markers.getLayers().length > 0) try {
							map.flyToBounds(markers.getBounds());
						} catch (e) {
							console.warn("Could not fly to bounds:", e);
						}
		...
		$(document).on("submit", ".contact-form", function(event) {
			...
			$button.closest("form").find("input[name=\"layout\"]").val($button.data("value"));
			...
			e.preventDefault();
			const $form = $(e.currentTarget);
			if ($form.data("pagination-click")) $form.removeData("pagination-click");
			else $form.find("input[name=\"page\"]").val("");
			...
			$.ajax({
				...
				complete: () => {
					...
					$("html, body").animate({ scrollTop: $dataListing.offset().top - 100 });
				}
			});
			...
			const $currentTarget = $(e.currentTarget);
			const $suggest = $currentTarget.closest("[data-bb-toggle=\"search-suggestion\"]").find("[data-bb-toggle=\"data-suggestion\"]");
			const cleanedFormData = cleanFormData($currentTarget.closest("form").serializeArray());
			...
			if ($hiddenInput.length > 0) $hiddenInput.val($currentTarget.data("value")).trigger("change");
			...
			if (!$(e.target).closest("[data-bb-toggle=\"data-suggestion\"]").length) $("[data-bb-toggle=\"data-suggestion\"]").slideUp();
		...
		$(`[data-bb-toggle="change-search-type"][data-value="${$(".flat-tab").find("form input[name=\"type\"]")}"]`).trigger("click");
		document.addEventListener("shortcode.loaded", (e) => {
			...
			switch (name) {
				...
				case "properties":
					initWishlist();
					if (attributes.style === "2") initPropertiesTab();
					if (attributes.style === "7") initProperties();
					break;
				...
				case "services":
					...
					break;
				case "projects":
					if (attributes.style === "2") initProjectsTab();
					break;
			}
		...
		const enhancedLocationDropdown = function() {
			...
			function enhanceLocationDropdown() {
				...
				if (!$locationNiceSelect.find(".nice-select-search-wrapper").length) {
					...
					if ($("#location").val()) $locationNiceSelect.addClass("has-value");
			...
			function setupEventHandlers($niceSelect) {
				...
				$niceSelect.on("click", function(e) {
					if ($niceSelect.hasClass("open") && !$(e.target).hasClass("nice-select-search")) setTimeout(() => $searchInput.focus(), 10);
				});
				...
				$.ajax({
					...
					success: function(response) {
						...
						if (typeof response === "string") {
							$("<div>").html(response).find(".search-suggestion-item").each(function() {
			...
			function updateCityOptions($niceSelect, cities, append) {
				...
				if (selectedVal && !$select.find("option[value=\"" + selectedVal + "\"]").length) $select.append("<option value=\"" + selectedVal + "\" selected>" + $niceSelect.find(".current").text() + "</option>");
				$list.find(".option").off("click").on("click", function(e) {
					...
					$niceSelect.toggleClass("has-value", val !== "");
					if (val) {
						if (!$select.find("option[value=\"" + val + "\"]").length) $select.append("<option value=\"" + val + "\">" + text + "</option>");
						$select.val(val);
					} else $select.val("");
					$list.find(".option").not("[data-value=\"\"]").remove();
		...
		function scrollToReviews() {
			...
			if ($reviewsSection.length) $("html, body").animate({ scrollTop: $reviewsSection.offset().top - 100 }, 800);
		}
		...
		const initHeroBannerSlider = function() {
			...
			const rotationInterval = $heroBanner.data("rotation-interval") || 5;
			if ($heroBanner.data("enable-rotation") !== "yes" || !sliderImages || sliderImages.length <= 1) return;
			let currentIndex = 0;
			let intervalId;
			const changeBackgroundImage = function() {
				currentIndex = (currentIndex + 1) % sliderImages.length;
				const nextImage = sliderImages[currentIndex];
				$heroBanner.css({ "background-image": `url('${nextImage}')` });
			};
			const startRotation = function() {
				intervalId = setInterval(changeBackgroundImage, rotationInterval * 1e3);
			};
		...
		const initCardImageSlider = function(el) {
			if (!el || el.dataset.swipeInit === "true" || typeof Swiper === "undefined") return;
			el.dataset.swipeInit = "true";
			el.addEventListener("click", function(e) {
				if (e.target.closest(".swiper-button-prev, .swiper-button-next, .swiper-pagination")) e.preventDefault();
			});
			new Swiper(el, {
				slidesPerView: 1,
				spaceBetween: 0,

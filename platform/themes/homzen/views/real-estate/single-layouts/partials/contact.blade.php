...
<div id="contact-form" @class(['widget-box single-property-contact', $class ?? null])>
    {{-- Contact Agency block: shown for projects only. Properties (especially
         MLS-imported listings) use the enquiry form alone; showing the local
         admin account as the agent would be misleading for IDX data. --}}
    @if ($isProject && ! RealEstateHelper::hideAgentInfoInPropertyDetailPage() && ($account = $model->author) && $account->exists)

<div @class(['wd-find-select' => in_array($style, [1, 2, 4]), 'wd-filter-select' => $style === 3, 'style-2 shadow-st' => $style === 2, 'no-left-round' => $noLeftRound ?? false]) style="width: 100%; min-height: 64px; height: 64px; padding: 8px 8px 8px 28px; border-radius: 14px; display: flex; align-items: center; justify-content: space-between; gap: 16px; background: #ffffff; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
    <div class="inner-group" style="flex: 1; margin: 0; padding: 0; border: none;">
        <div class="form-group-1 form-search-form form-style form-search-keyword-input w-100" data-bb-toggle="search-suggestion" style="padding: 0; margin: 0; border: none; background: transparent;">
            <div class="position-relative w-100">
                <input type="text" class="form-control" placeholder="{{ __('Search projects, condos, location...') }}" value="{{ BaseHelper::stringify(request()->query('k')) }}" name="k" style="height: 48px; border: none; background: transparent; font-size: 15px; width: 100%; box-shadow: none; outline: none; padding: 0 10px 0 0;" />
                <div data-bb-toggle="data-suggestion"></div>
            </div>
        </div>
    </div>
    <button type="submit" class="tf-btn primary" style="height: 48px; min-width: 140px; padding: 0 28px; border-radius: 10px; font-weight: 600; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; gap: 6px; font-size: 15px;">
